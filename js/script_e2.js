...
.done(function(data, textStatus, jqXHR) {
    ...
    if (jqXHR.status == 200) {
      ...
      var trip = data['trip'];
      if(trip == 'EGAT'){
        document.getElementById("EGAT").checked = true;
      }else{
        document.getElementById("PEA").checked = true;
      }
...
function validateForm(){
  var triptype = document.forms['form_e2'].elements['trip'];
  var trip = "";
  var i;
  for (i = 0; i < triptype.length; i++) {
    if (triptype[i].checked) {
  ...
  jQuery.ajax({
    ...
    dataType: 'json',
    data: JSON.stringify({
      "peacode" : document.forms["form_e2"]["peacode"].value,
