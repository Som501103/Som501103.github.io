...
.done(function(data, textStatus, jqXHR) {
    ...
    if (jqXHR.status == 200) {
      ...
      console.log(data.equipcode)
      document.getElementById("equipcode").value= equipcode;
      // var fedderwork = data['electricdata']['fedderwork'];
      document.getElementById("fedderwork").value = data.fedderwork;
      // var equipend = data['electricdata']['equipend'];
      document.getElementById("equipend").value = data.equipend;
      var load = data['load'];
      document.getElementById("load").value = load;
      // var fault_current = data['fault_current'];
...
var formData = new FormData($("#form_e1")[0]);
...
// formData.set("electrician", data.electrician)
...
$(document).ready(function() {
  ...
  display: "อื่นๆ",value: "CC12001"}];
  // Function executes on change of first select option field.
  $("#causetype").change(function() {
  
  var select = $("#causetype option:selected").val();
  switch (select) {
  case "TC001":
  subcause(TC001);
  break;
  ...
  default:
  $("#subcause").empty();
  $("#subcause").append("<option>กรุณาเลือกสาเหตุ</option>");
  break;
  }
  });
  // Function To List out Cities in Second Select tags
  function subcause(arr) {
  $("#subcause").empty(); //To reset cities
  $("#subcause").append("<option>กรุณาเลือกสาเหตุย่อย</option>");
  $(arr).each(function(i) { //to list cities
  $("#subcause").append("<option value="+ arr[i].value + ">" + arr[i].display + "</option>")
  });
  }
  });
